...
def update_book(book_path, output_path, mapping):
    """
    Phase 2: Reads the book, identifies subsection blocks, replaces placeholders
    with 🏷️-prefixed terms inside a ::: expand 🔗 block, or deletes the placeholder
    entirely if no terms exist for that section.
    """
    ...
    # Regex matches the section header and captures the entire '::: expand 🏷️ ... :::' block
    ...
    def replace_placeholder(match):
        ...
        if anchor_id in mapping and mapping[anchor_id]:
            ...
            # Construct the new block using the 🔗 emoji
            new_block = "::: expand 🔗\n"
            for term in terms:
                slug = (
                    term.lower()
                ...
                # Prepends each item link with the 🏷️ emoji and a space
                new_block += f"🏷️ [{term}](#{slug})\n"
            new_block += ":::"
            return header_and_body + new_block

        # If no terms are found, return ONLY the body text (deletes the original placeholder)
        return header_and_body.rstrip() + "\n"
...
if __name__ == "__main__":
    ...
    if index_mapping:
        ...
        print("Injecting ordered terms and updating container formatting...")
